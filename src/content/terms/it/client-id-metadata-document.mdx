---
title: Documento dei metadati del Client ID (Client ID Metadata Document, CIMD)
tags: [oauth 2.0]
description: Un Documento dei metadati del Client ID (Client ID Metadata Document, CIMD) è un documento JSON ospitato a un URL HTTPS che funge da identificatore del client OAuth 2.0. Permette ai client di identificarsi ai authorization server senza registrazione preventiva, utilizzando l'URL come client_id.
---

## Che cos'è un Documento dei metadati del Client ID (Client ID Metadata Document, CIMD)?

Un Documento dei metadati del Client ID (Client ID Metadata Document, CIMD) è un meccanismo definito nella specifica [OAuth Client ID Metadata Document](https://datatracker.ietf.org/doc/draft-ietf-oauth-client-id-metadata-document/) che consente a un <Ref slug="client" /> OAuth 2.0 di identificarsi presso un <Ref slug="authorization-server" /> senza registrazione preventiva.
...
Quando un client utilizza un Documento dei metadati del Client ID (Client ID Metadata Document, CIMD), il flusso OAuth aggiunge un passaggio: il authorization server risolve l'URL `client_id` per recuperare i metadati del client.
...
### Vantaggi del Documento dei metadati del Client ID (Client ID Metadata Document, CIMD)

L'approccio del Documento dei metadati del Client ID (Client ID Metadata Document, CIMD) affronta questi problemi:
...
I authorization server indicano il supporto per i Documenti dei metadati del Client ID (Client ID Metadata Document, CIMD) includendo la seguente proprietà nei loro <Ref slug="authorization-server-metadata" />:
