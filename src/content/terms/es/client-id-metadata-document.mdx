---
title: Documento de Metadatos de Client ID (Client ID Metadata Document)
tags: [oauth 2.0]
description: Un Documento de Metadatos de Client ID (Client ID Metadata Document) es un documento JSON alojado en una URL HTTPS que sirve como identificador de cliente de OAuth 2.0. Permite que los clientes se identifiquen ante los authorization servers sin registro previo utilizando la URL como el client_id.
---

## ¿Qué es un Documento de Metadatos de Client ID (Client ID Metadata Document)?

Un Documento de Metadatos de Client ID (Client ID Metadata Document) es un mecanismo definido en la especificación [OAuth Client ID Metadata Document](https://datatracker.ietf.org/doc/draft-ietf-oauth-client-id-metadata-document/) que permite que un <Ref slug="client" /> de OAuth 2.0 se identifique ante un <Ref slug="authorization-server" /> sin registro previo.
...
Cuando un client utiliza un Documento de Metadatos de Client ID (Client ID Metadata Document), el flujo de OAuth agrega un paso: el authorization server resuelve la URL de `client_id` para recuperar los metadatos del client.
...
### Ventajas del Documento de Metadatos de Client ID (Client ID Metadata Document)

El enfoque de Documento de Metadatos de Client ID (Client ID Metadata Document) aborda estos problemas:
