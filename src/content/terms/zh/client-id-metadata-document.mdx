...
sequenceDiagram
    ...
    Client->>AS: 授权请求 (authorization request)<br/>`client_id=https://client.example.com/metadata`
    AS->>Client: 从 client_id URL 获取元数据
    Client-->>AS: 返回 JSON 元数据文档
    AS->>User: 显示带有元数据中客户端信息的同意界面
    User->>AS: 授予授权
    AS->>Client: 重定向并携带授权码
```
