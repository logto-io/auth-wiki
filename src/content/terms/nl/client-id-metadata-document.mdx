---
title: Client ID Metadata Document (Client ID Metadata Document)
tags: [oauth 2.0]
description: Een Client ID Metadata Document (Client ID Metadata Document) is een JSON-document dat wordt gehost op een HTTPS-URL en dient als de OAuth 2.0 client identifier. Hiermee kunnen clients zichzelf identificeren bij authorization servers (authorization servers) zonder voorafgaande registratie door de URL als client_id te gebruiken.
---

## Wat is een Client ID Metadata Document (Client ID Metadata Document)?

Een Client ID Metadata Document (Client ID Metadata Document) is een mechanisme gedefinieerd in de [OAuth Client ID Metadata Document](https://datatracker.ietf.org/doc/draft-ietf-oauth-client-id-metadata-document/) specificatie waarmee een OAuth 2.0 <Ref slug="client" /> zichzelf kan identificeren bij een <Ref slug="authorization-server" /> zonder voorafgaande registratie.
...
Wanneer een client een Client ID Metadata Document (Client ID Metadata Document) gebruikt, voegt de OAuth-flow één stap toe: de authorization server resolveert de `client_id` URL om de metadata van de client op te halen.
...
### Voordelen van Client ID Metadata Document (Client ID Metadata Document)

De Client ID Metadata Document (Client ID Metadata Document) aanpak pakt deze problemen aan:

| Aspect | Statische registratie | DCR | Client ID Metadata Document (Client ID Metadata Document) |
|--------|----------------------|-----|----------------------------|
...
Authorization servers geven aan dat ze Client ID Metadata Documents (Client ID Metadata Documents) ondersteunen door de volgende property op te nemen in hun <Ref slug="authorization-server-metadata" />:
...
## Client ID Metadata Document Services (Client ID Metadata Document Services)

De specificatie definieert ook **Client ID Metadata Document Services (Client ID Metadata Document Services)** — externe webdiensten die metadata documenten hosten namens ontwikkelaars.

Dit vult een praktische leemte: tijdens lokale ontwikkeling hebben ontwikkelaars geen publiek toegankelijke URL om hun metadata te hosten. Een Client ID Metadata Document Service (Client ID Metadata Document Service) biedt een stabiele publieke URL die authorization servers kunnen ophalen, terwijl de ontwikkelaar lokaal werkt. Dit voorkomt dat lokale machines aan het internet moeten worden blootgesteld of tunnels moeten worden opgezet om OAuth-flows te testen.
