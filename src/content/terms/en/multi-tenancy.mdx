...
Authentication and authorization alone won’t prevent a user with the right role from accessing another tenant’s resources. To limit access, we need to add a “tenant” context, like a tenant ID, Org ID, or Workspace ID. These identifiers act like walls, doors, and locks, keeping tenants separate.

The term “organization” is commonly used for tenant isolation. Many Customer Identity Management (CIAM) or Auth services include an “organization” feature to help isolate tenants while keeping a unified identity system. The user inside the organization is called "member".
