...
tags: [oauth 2.0]
description: Client ID Metadata Document (Client ID Metadata Document) 是一個託管於 HTTPS URL 的 JSON 文件，作為 OAuth 2.0 client identifier（client 標識符）。它允許 client（client）在未事先註冊的情況下，使用該 URL 作為 client_id，向 authorization server（authorization server）進行身份識別。
---

## 什麼是 Client ID Metadata Document (Client ID Metadata Document)？

Client ID Metadata Document (Client ID Metadata Document) 是 [OAuth Client ID Metadata Document](https://datatracker.ietf.org/doc/draft-ietf-oauth-client-id-metadata-document/) 規範中定義的一種機制，允許 OAuth 2.0 <Ref slug="client" /> 在未事先註冊的情況下，向 <Ref slug="authorization-server" /> 進行身份識別。
...
當 client（client）使用 Client ID Metadata Document（Client ID Metadata Document）時，OAuth 流程會多一個步驟：authorization server（authorization server）會解析 `client_id` URL 並取得 client（client）的 metadata（metadata）。
...
### Client ID Metadata Document（Client ID Metadata Document）優勢

Client ID Metadata Document（Client ID Metadata Document）解決了上述問題：

| 面向 | Static registration（static registration） | DCR | Client ID Metadata Document（Client ID Metadata Document） |
|--------|-------------------|-----|----------------------------|
...
authorization server（authorization server）會在其 <Ref slug="authorization-server-metadata" /> 中加入下列屬性，表示支援 Client ID Metadata Document（Client ID Metadata Document）：
...
規範也定義了 **Client ID Metadata Document Services（Client ID Metadata Document Services）**——第三方網路服務，代開發者託管 metadata document（metadata 文件）。

這解決了一個實務問題：本地開發時，開發者通常沒有公開可存取的 URL 來託管 metadata。Client ID Metadata Document Service（Client ID Metadata Document Service）可提供一個穩定的公開 URL，讓 authorization server（authorization server）能抓取 metadata，而開發者仍可在本地作業。這樣就不必將本機暴露到網際網路，或為測試 OAuth 流程而架設 tunnel。
