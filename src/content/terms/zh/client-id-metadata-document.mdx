---
title: 客户端 ID 元数据文档 (Client ID Metadata Document)
tags: [oauth 2.0]
description: 客户端 ID 元数据文档 (Client ID Metadata Document) 是一个托管在 HTTPS URL 上的 JSON 文档，作为 OAuth 2.0 client identifier（客户端标识符）。它允许客户端通过使用 URL 作为 client_id，无需预先注册即可向 authorization server（授权服务器）标识自己。
---

## 什么是客户端 ID 元数据文档 (Client ID Metadata Document)？

客户端 ID 元数据文档 (Client ID Metadata Document) 是 [OAuth Client ID Metadata Document](https://datatracker.ietf.org/doc/draft-ietf-oauth-client-id-metadata-document/) 规范中定义的一种机制，允许 OAuth 2.0 <Ref slug="client" /> 在无需预先注册的情况下向 <Ref slug="authorization-server" /> 标识自己。
...
当客户端使用客户端 ID 元数据文档 (Client ID Metadata Document) 时，OAuth 流程会多出一步：授权服务器会解析 `client_id` URL 以获取客户端的元数据。
...
### 客户端 ID 元数据文档 (Client ID Metadata Document) 的优势

客户端 ID 元数据文档 (Client ID Metadata Document) 方式解决了上述问题：

| 方面 | 静态注册 | DCR | 客户端 ID 元数据文档 (Client ID Metadata Document) |
|--------|-------------------|-----|----------------------------|
...
授权服务器通过在其 <Ref slug="authorization-server-metadata" /> 中包含以下属性，表明支持客户端 ID 元数据文档 (Client ID Metadata Document)：
