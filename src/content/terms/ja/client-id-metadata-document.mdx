---
title: クライアント ID メタデータドキュメント (Client ID Metadata Document, CIMD)
tags: [oauth 2.0]
description: クライアント ID メタデータドキュメント (Client ID Metadata Document, CIMD) は、OAuth 2.0 のクライアント識別子として機能する HTTPS URL でホストされる JSON ドキュメントです。これにより、クライアントは事前登録なしで URL を client_id として使用し、authorization server に自身を識別できます。
---

## クライアント ID メタデータドキュメント (Client ID Metadata Document, CIMD) とは？

クライアント ID メタデータドキュメント (Client ID Metadata Document, CIMD) は、[OAuth Client ID Metadata Document](https://datatracker.ietf.org/doc/draft-ietf-oauth-client-id-metadata-document/) 仕様で定義された仕組みであり、OAuth 2.0 <Ref slug="client" /> が事前登録なしで <Ref slug="authorization-server" /> に自身を識別できるようにします。
...
クライアントがクライアント ID メタデータドキュメント (Client ID Metadata Document, CIMD) を使用する場合、OAuth フローに 1 ステップ追加されます：authorization server が `client_id` の URL を解決し、クライアントのメタデータを取得します。
...
### クライアント ID メタデータドキュメント (Client ID Metadata Document, CIMD) の利点

クライアント ID メタデータドキュメント (Client ID Metadata Document, CIMD) アプローチはこれらの課題を解決します：
...
authorization server は、<Ref slug="authorization-server-metadata" /> に以下のプロパティを含めることでクライアント ID メタデータドキュメント (Client ID Metadata Document, CIMD) のサポートを示します：
