---
title: 클라이언트 ID 메타데이터 문서 (Client ID Metadata Document)
tags: [oauth 2.0]
description: 클라이언트 ID 메타데이터 문서 (Client ID Metadata Document)는 OAuth 2.0 클라이언트 식별자로 사용되는 HTTPS URL에 호스팅된 JSON 문서입니다. 이 문서를 통해 클라이언트는 URL을 client_id로 사용하여 사전 등록 없이 authorization server(인가 서버)에 자신을 식별할 수 있습니다.
---

## 클라이언트 ID 메타데이터 문서 (Client ID Metadata Document)란?

클라이언트 ID 메타데이터 문서 (Client ID Metadata Document)는 [OAuth Client ID Metadata Document](https://datatracker.ietf.org/doc/draft-ietf-oauth-client-id-metadata-document/) 명세에서 정의된 메커니즘으로, OAuth 2.0 <Ref slug="client" />가 사전 등록 없이 <Ref slug="authorization-server" />에 자신을 식별할 수 있도록 해줍니다.
...
클라이언트가 클라이언트 ID 메타데이터 문서 (Client ID Metadata Document)를 사용할 때, OAuth 플로우에 한 단계가 추가됩니다: authorization server(인가 서버)가 `client_id` URL을 해석하여 클라이언트의 메타데이터를 가져옵니다.
...
### 클라이언트 ID 메타데이터 문서 (Client ID Metadata Document)의 장점

클라이언트 ID 메타데이터 문서 (Client ID Metadata Document) 방식은 다음과 같은 문제를 해결합니다:
...
authorization server(인가 서버)는 <Ref slug="authorization-server-metadata" />에 다음 속성을 포함하여 클라이언트 ID 메타데이터 문서 (Client ID Metadata Document) 지원 여부를 표시합니다:
