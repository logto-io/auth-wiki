...
tags: [oauth 2.0]
description: Um Documento de Metadados de Client ID (Client ID Metadata Document, CIMD) é um documento JSON hospedado em uma URL HTTPS que serve como o identificador de client do OAuth 2.0. Ele permite que clients se identifiquem para authorization servers sem registro prévio, usando a URL como o client_id.
---

## O que é um Documento de Metadados de Client ID (Client ID Metadata Document, CIMD)?

Um Documento de Metadados de Client ID (Client ID Metadata Document, CIMD) é um mecanismo definido na especificação [OAuth Client ID Metadata Document](https://datatracker.ietf.org/doc/draft-ietf-oauth-client-id-metadata-document/) que permite que um OAuth 2.0 <Ref slug="client" /> se identifique para um <Ref slug="authorization-server" /> sem registro prévio.
