---
title: Documento de Metadados de Client ID (Client ID Metadata Document, CIMD)
tags: [oauth 2.0]
description: Um Documento de Metadados de Client ID (Client ID Metadata Document, CIMD) é um documento JSON hospedado num URL HTTPS que serve como identificador de client OAuth 2.0. Permite que os clients se identifiquem perante os authorization servers sem registo prévio, utilizando o URL como o client_id.
---

## O que é um Documento de Metadados de Client ID (Client ID Metadata Document, CIMD)?

Um Documento de Metadados de Client ID (Client ID Metadata Document, CIMD) é um mecanismo definido na especificação [OAuth Client ID Metadata Document](https://datatracker.ietf.org/doc/draft-ietf-oauth-client-id-metadata-document/) que permite a um OAuth 2.0 <Ref slug="client" /> identificar-se perante um <Ref slug="authorization-server" /> sem registo prévio.
...
Quando um client utiliza um Documento de Metadados de Client ID (Client ID Metadata Document, CIMD), o fluxo OAuth adiciona um passo: o authorization server resolve o URL do `client_id` para obter os metadados do client.
...
A abordagem do Documento de Metadados de Client ID (Client ID Metadata Document, CIMD) resolve estas questões:

| Aspeto | Registo estático | DCR | Documento de Metadados de Client ID (Client ID Metadata Document, CIMD) |
|--------|-------------------|-----|----------------------------|
