...
title: Claim (Claim)
tags: [oauth 2.0, oidc]
description: Een claim (claim) in JSON Web Token (JWT) is een naam-waarde paar dat specifieke informatie overbrengt. In een bredere context kan een claim (claim) elk naam-waarde paar zijn dat informatie vertegenwoordigt.
---

## Wat is een claim (claim)?

Een claim (claim) is een naam-waarde paar dat specifieke informatie overbrengt. Bijvoorbeeld, in een JSON-object:
...
In dit voorbeeld is `sub` een claim (claim) die de uniek identifier van het onderwerp vertegenwoordigt met de waarde `foo`, en `email` is een claim (claim) die het e-mailadres vertegenwoordigt met de waarde `foo@bar.com`.
...
### Gebruik van claims (claims)

Claims (claims) worden op grote schaal gebruikt in verschillende contexten, zoals:

- <Ref slug="jwt" />: In een JWT worden claims (claims) gebruikt om informatie over het token te vertegenwoordigen, zoals de issuer (`iss`), subject (`sub`) en vervaltijd (`exp`).
- <Ref slug="openid-connect" />: In OIDC worden claims (claims) gebruikt om gebruikersinformatie te vertegenwoordigen in de <Ref slug="id-token" /> en <Ref slug="userinfo-endpoint" />, zoals het e-mailadres van de gebruiker (`email`), naam (`name`) en profielfoto (`picture`).
