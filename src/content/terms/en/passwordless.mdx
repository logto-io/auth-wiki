...
2. **Something the user has**, such as a mobile phone, <Ref slug='otp'>one-time password (OTP)</Ref>, or hardware token.
...
Passwordless authentication and <Ref slug='mfa'>multi-factor authentication (MFA)</Ref> are sometimes mentioned in similar contexts. Therefore, it is necessary to define these two terms separately and understand the difference between them:
...
Based on the previous definition of passwordless, <Ref slug='single-sign-on'>Single sign-on (SSO)</Ref> is one type of passwordless method (users has/owns an account for SSO provider).
