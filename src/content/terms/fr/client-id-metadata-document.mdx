---
title: Document de métadonnées d'identifiant client (Client ID Metadata Document, CIMD)
tags: [oauth 2.0]
description: Un document de métadonnées d'identifiant client (Client ID Metadata Document, CIMD) est un document JSON hébergé à une URL HTTPS qui sert d'identifiant client OAuth 2.0. Il permet aux clients de s'identifier auprès des serveurs d'autorisation sans enregistrement préalable en utilisant l'URL comme client_id.
---

## Qu'est-ce qu'un document de métadonnées d'identifiant client (Client ID Metadata Document, CIMD) ?

Un document de métadonnées d'identifiant client (Client ID Metadata Document, CIMD) est un mécanisme défini dans la spécification [OAuth Client ID Metadata Document](https://datatracker.ietf.org/doc/draft-ietf-oauth-client-id-metadata-document/) qui permet à un <Ref slug="client" /> OAuth 2.0 de s'identifier auprès d'un <Ref slug="authorization-server" /> sans enregistrement préalable.
...
Lorsqu'un client utilise un document de métadonnées d'identifiant client (Client ID Metadata Document, CIMD), le flux OAuth ajoute une étape : le serveur d'autorisation résout l'URL `client_id` pour récupérer les métadonnées du client.
...
### Avantages du document de métadonnées d'identifiant client (Client ID Metadata Document, CIMD)

L'approche du document de métadonnées d'identifiant client (Client ID Metadata Document, CIMD) répond à ces problèmes :
...
Les serveurs d'autorisation indiquent la prise en charge des documents de métadonnées d'identifiant client (Client ID Metadata Document, CIMD) en incluant la propriété suivante dans leur <Ref slug="authorization-server-metadata" /> :
