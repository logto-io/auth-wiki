---
title: Metadados do Servidor de Autorização (Authorization Server Metadata)
tags: [oauth 2.0, oidc]
description: Os Metadados do Servidor de Autorização OAuth 2.0 são um formato padronizado que permite que clientes OAuth 2.0 descubram e obtenham automaticamente as informações necessárias para interagir com servidores de autorização OAuth 2.0, incluindo locais de endpoints e capacidades do servidor.
---

## O que são Metadados do Servidor de Autorização OAuth 2.0?

Os Metadados do Servidor de Autorização OAuth 2.0 são um formato padronizado definido no [RFC 8414](https://datatracker.ietf.org/doc/html/rfc8414) que permite que clientes OAuth 2.0 descubram e obtenham automaticamente as informações necessárias para interagir com servidores de autorização OAuth 2.0.
...
1. O cliente conhece ou descobre a URL do issuer do servidor de autorização
2. O cliente constrói a URL de metadados anexando `/.well-known/oauth-authorization-server` à URL do issuer
3. O cliente recupera o documento de metadados contendo a configuração do servidor
4. O cliente usa os metadados recuperados (endpoints, recursos suportados, etc.) para se configurar adequadamente e interagir com o servidor de autorização

Por exemplo, se a URL do issuer for `https://auth.example.com`, os metadados estariam disponíveis em:
...
- `authorization_endpoint`: A URL do endpoint de autorização
- `token_endpoint`: A URL do endpoint de token
- `scopes_supported`: Scopes disponíveis
- `response_types_supported`: Tipos de resposta suportados
- `token_endpoint_auth_methods_supported`: Métodos de autenticação de cliente suportados
...
1. **Configuração inicial**:
   ...
   - O cliente verifica `response_types_supported` para garantir que seu tipo de fluxo (por exemplo, `code`) seja suportado

2. **Solicitação de autorização (authorization request)**:
   - O cliente usa `authorization_endpoint` para construir a URL de autorização
   - O cliente seleciona scopes apropriados de `scopes_supported` com base em suas necessidades
   - Exemplo: `https://auth.example.com/authorize?response_type=code&scope=profile email`

3. **Troca de token**:
   - Após receber o authorization code, o cliente usa `token_endpoint` para a troca de token
...
1. **Escopo e Propósito**:
   ...
   - <Ref slug="openid-connect-discovery"/> inclui parâmetros adicionais para recursos relacionados à identidade, como:
     ...
     - Inclui tipos de resposta de fluxo híbrido (por exemplo, `code id_token`, `code id_token token`)
...
2. Servidores de autorização podem suportar ambos os endpoints simultaneamente
3. Provedores de OpenID geralmente expõem ambos os endpoints para compatibilidade retroativa
