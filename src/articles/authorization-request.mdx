...
- **Resource owner password credentials (ROPC) flow**: A flow (grant) where the user's credentials are exchanged directly for an access token. This flow is not recommended due to security risks.
