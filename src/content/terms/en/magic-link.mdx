...
Let's assume the user uses email to receive a magic link or <Ref slug='otp'>one-time password (OTP)</Ref>.

In many cases, users use email to receive an OTP to verify login or as an additional verification factor for <Ref slug='mfa'>multi-factor authentication (MFA)</Ref>. What is the difference between using an email magic link and an email OTP?
