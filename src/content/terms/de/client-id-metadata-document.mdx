...
Wenn ein Client ein Client ID Metadata Document (Client ID Metadata Document) verwendet, fügt der OAuth-Flow einen Schritt hinzu: Der Authorization Server löst die `client_id`-URL auf, um die Metadaten des Clients abzurufen.
...
sequenceDiagram
    ...
    Client->>AS: Authorization request (Authorization Request) mit<br/>`client_id=https://client.example.com/metadata`
...
### Vorteile des Client ID Metadata Document (Client ID Metadata Document)

Der Ansatz des Client ID Metadata Document (Client ID Metadata Document) adressiert diese Probleme:
...
Authorization Server signalisieren die Unterstützung für Client ID Metadata Documents (Client ID Metadata Documents), indem sie folgende Eigenschaft in ihre <Ref slug="authorization-server-metadata" /> aufnehmen:
